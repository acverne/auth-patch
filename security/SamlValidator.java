...
public class SamlValidator extends BusModBase implements Handler<Message<JsonObject>> {
	...
	public void generateSAMLResponse(final String serviceProvider, final String authNRequestId, final String userId, final String nameId,
		...
		final String entngIdpNameQualifier = "https://" + host + "/auth/saml/metadata/idp.xml"; // Patched by Micorksen: the format of the IDP Name qualifier is the same for EVERY domain hosted on the ENT.
		...
		createVectors(userId, host, url, new Handler<Either<String, JsonArray>>() {
			...
			public void handle(Either<String, JsonArray> event) {
				if (event.isRight()) {
					...
					String _nameId = nameId;
					if (attributes.containsKey("NameID")) { // Patched by Micorksen: Allow the services to provide a custom NameID instead of the one generated by ENTCore and don't include it in the <AttributeStatement>.
						_nameId = attributes.get("NameID").get(0);
						attributes.remove("NameID");
					}
					...
					try {
						assertion = generateAssertion(authNRequestId, entngIdpNameQualifier, serviceProvider, _nameId,
	...
	private Assertion generateAssertion(String authNRequestId, String idp, String serviceProvider, String nameId, String recipient,
		...
		issuerAudience.setAudienceURI(serviceProvider.startsWith("google.com/a/") || serviceProvider.startsWith("https://accounts.google.com/samlrp/") ? recipient : serviceProvider); // Patched by Micorksen: Set the ACS url if the SAML request is from Google Workspace.
